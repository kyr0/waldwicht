...
**Future Research.** A planned extension beyond Waldwicht V1 is a deployment-oriented adaptation pipeline that combines AWQ, Waldwicht quantization, DWQ-based recovery, QLoRA fine-tuning, adapter fusion, and final re-quantization into a single compact checkpoint. The intended workflow starts from a BF16 Gemma 4 base model, applies AWQ and Waldwicht quantization, uses DWQ to recover quality lost to compression, performs lightweight behavioral adaptation with QLoRA, fuses the learned adapters back into a higher-precision parent model, and then re-applies Waldwicht quantization, optionally followed by a short DWQ refresh. The objective is to retain the convenience of adapter training while producing a standalone quantized model for inference without external adapter dependencies.

We also work on building `autokernel-mlx`, an agentic, open-source system for generating, benchmarking, and compiling custom Metal kernels for Apple Silicon inference. It targets three bottlenecks exposed by this study: dynamic precision at inference time, fused mixed-precision dequantization kernels, and KV-cache quantization. The aim is to turn quantization gains into real inference-time wins by reducing memory traffic, improving throughput, and lowering runtime memory cost. `autokernel-mlx` is planned to follow an LLM-in-the-loop generate-evaluate-refine workflow for kernel synthesis and optimization. See section 12 for details.
...
A first direction for future work is a deployment-oriented post-training adaptation pipeline that extends Waldwicht beyond its current V1 scope. In this setting, a BF16 Gemma 4 base model would first be preconditioned with AWQ, then quantized with Waldwicht, refined with DWQ to recover quantization-induced degradation, adapted with QLoRA for task-specific behaviors, fused back into a higher-precision parent model, and finally re-quantized with Waldwicht, optionally followed by a short DWQ refresh. The aim of this pipeline is to retain the flexibility of adapter-based fine-tuning during development while producing a single compact checkpoint for deployment without external adapters. Although this workflow is not part of Waldwicht V1, it represents a natural next step toward integrating compression, recovery, and lightweight post-training adaptation within a unified deployment pipeline.
