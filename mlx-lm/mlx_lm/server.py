    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            if cache is None:
                turbo_kwargs = {}
                if getattr(self.cli_args, "turbo_kv_bits", None) is not None:
                    turbo_kwargs["turbo_kv_bits"] = self.cli_args.turbo_kv_bits
                    turbo_kwargs["turbo_fp16_layers"] = self.cli_args.turbo_fp16_layers
                cache = make_prompt_cache(
                    self.model_provider.model, **turbo_kwargs
                )
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        """
        if self.path == "/debug/clear_cache":
    ...
    def handle_debug_memory(self):
        """Return MLX memory stats and prompt cache info."""
        ...
        data = {
            ...
            "peak_bytes": peak,
            "cache_bytes": cache,
            "active_mb": round(active / 1048576, 1),
            "peak_mb": round(peak / 1048576, 1),
            "cache_mb": round(cache / 1048576, 1),
            "prompt_cache_entries": pc_entries,
            "prompt_cache_mb": round(pc_bytes / 1048576, 1),
        }
        self._set_completion_headers(200)
        self.end_headers()
    ...
    def handle_clear_cache(self):
        """Clear the MLX buffer cache and return before/after stats."""
        import json as _json

        before_cache = mx.get_cache_memory()
        before_active = mx.get_active_memory()
        mx.clear_cache()
        after_cache = mx.get_cache_memory()
        ...
        data = {
            ...
            "freed_bytes": freed,
            "before_cache_mb": round(before_cache / 1048576, 1),
            "after_cache_mb": round(after_cache / 1048576, 1),
            "freed_mb": round(freed / 1048576, 1),
            "active_mb": round(after_active / 1048576, 1),
        }
        self._set_completion_headers(200)
        self.end_headers()
        self.wfile.write(_json.dumps(data).encode())
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="TurboQuant KV cache compression bits (1-4). 3-bit gives 4.6x compression.",
    )
    parser.add_argument(
        "--turbo-fp16-layers",
        type=int,
        default=2,
        help="Number of first/last layers to keep in FP16 with TurboQuant (default: 2)",
    )
    parser.add_argument(
        "--prompt-cache-size",
        type=int,
